...
@WebServlet("/PopulateTable1")
public class PopulateTable1 extends HttpServlet 
{
		 ...
		 ) throws ServletException, IOException 
 {
	 ...
	 if (session != null) 
	{
		 ...
		 country=FetchData.getAllCountries(getShipName,getOrderNo);
   ...
   // String  i = request.getParameter("fromdate");
	...
	if(myJsonData !=null && myJsonData.length > 0)	 
	{
		checkedValue ="";
		 for (int m = 0; m < myJsonData.length; m++) 
		 {
			...
			// System.out.println ( "Approved Checked " +  checkedValue );
			 FetchData.saveOrUpdateAppChecked(checkedValue);   
		 }
	 }
	 ...
	 { 
		 ...
		 for (int m = 0; m < myJsonData1.length; m++) 
		 {
			...
			// System.out.println ("Approved Unchecked " + uncheckedValue );
			 FetchData.saveOrUpdateAppUnChecked(uncheckedValue);   
		 }
	 }
 	 ...
 	 { 
		 checkedValue ="";
 		 for (int m = 0; m < myJsonData2.length; m++) 
 	     {
 	       ...
 	       FetchData.saveOrUpdateRejChecked(checkedValue);  
 	     }
 	 }
 	 ...
 	 { 
 		uncheckedValue ="";
 		 for (int m = 0; m < myJsonData3.length; m++) 
 	     {
 	       ...
 	       FetchData.saveOrUpdateRejUnChecked(uncheckedValue);  
 	     }
 	 }
 	...
 	if (orderno != null && orderdate!= null)
 	{
 		...
 		FetchData.saveOrUpdateOrderStatus(orderno,orderdate,n);
 	}
